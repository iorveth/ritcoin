...
pub fn private_key_to_WIF(key: String) -> Result<String, RitCoinErrror> {
...
pub fn verify(hash: &[u8], signature: &[u8], public_key: &[u8]) -> Result<(), secp256k1::Error> {
    let secp = Secp256k1::new();
    let hash = Message::from_slice(&[0xab; 32])?;
...
pub fn get_address(public_key: &[u8]) -> Result<String, RitCoinErrror> {
